                        ...
                        @forelse($barangs as $item)
                            <tr class="border-bottom theme-border">
                                ...
                                <td class="text-end pe-4">
                                    ...
                                    @if(Auth::check() && Auth::user()->role === 'admin')
                                        <a href="{{ route('barang.edit', $item->id_barang ?? $item->id) }}" class="btn btn-warning btn-sm me-1 fw-bold">
                                            <i class="bi bi-pencil-square"></i> Edit
                                        </a>
                                        
                                        <form action="{{ route('barang.destroy', $item->id_barang ?? $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin mau hapus barang ini?')">
                                            @csrf
                                            @method('DELETE')
